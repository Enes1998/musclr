// Domain types and data — ported verbatim from the prototype (frontend/src/lib/exercises.ts).
...
export interface WorkoutEntry {
  ...
  reps: number;
  weight: number;
  /** Reps in reserve (0 = to failure). Optional — informs the AI, not the frozen scoring. */
  rir?: number;
  /** Rating of perceived exertion (6-10). Optional. */
  rpe?: number;
  /** Unit the `weight` is recorded in. Defaults to kg (scoring thresholds assume kg). */
  unit?: WeightUnit;
}
