// Scoring engine — ported verbatim from prototype data.js
 ...
 */
export function computeMuscleLoad(week: WeekData): Record<MuscleId, number> {
  const totals: Record<string, number> = {};
  MUSCLE_GROUPS.forEach(m => { totals[m.id] = 0; });

  const repFactor = (r: number) => r <= 6 ? 1.0 : r <= 12 ? 0.85 : 0.7;
  const wFactor = (w: number) => w >= 100 ? 1.3 : w >= 40 ? 1.0 : w > 0 ? 0.85 : 0.7;
  ...
  Object.entries(totals).forEach(([m, v]) => {
    norm[m as MuscleId] = Math.max(0, Math.min(100, Math.round((v / 30) * 100)));
  });
  return norm;
}
...
export function scoreToColor(score: number): string {
  ...
  if (s < 50) {
    const t = s / 50;
    return hex(lerp(GREEN[0], YELLOW[0], t), lerp(GREEN[1], YELLOW[1], t), lerp(GREEN[2], YELLOW[2], t));
  } else {
    const t = (s - 50) / 50;
    return hex(lerp(YELLOW[0], RED[0], t), lerp(YELLOW[1], RED[1], t), lerp(YELLOW[2], RED[2], t));
  }
}
